...
public class FileProgressMonitor extends TimerTask implements SftpProgressMonitor {
    ...
    public FileProgressMonitor(long fileSize,WebSocketSession webSocketSession) {
    ...
    public void run() {
        if (!isEnd()) { // 判断传输是否已结束
        	 //WebSocketMessageSend.sendMessage(webSocketSession, "Transfering is in progress.");
            System.out.println("Transfering is in progress.");
            long transfered = getTransfered();
            if (transfered != fileSize) { // 判断当前已传输数据大小是否等于文件总大小
            	 WebSocketMessageSend.sendMessage(webSocketSession,"Current transfered: " + transfered + " bytes");
                System.out.println("Current transfered: " + transfered + " bytes");
                sendProgressMessage(transfered);
            } else {
            	//WebSocketMessageSend.sendMessage(webSocketSession,"File transfering is done.");
                System.out.println("File transfering is done.");
                setEnd(true); // 如果当前已传输数据大小等于文件总大小，说明已完成，设置end
            }
        } else {
        	//WebSocketMessageSend.sendMessage(webSocketSession,"Transfering done.");
    ...
    private void sendProgressMessage(long transfered) {
        if (fileSize != 0) {
            ...
            WebSocketMessageSend.sendMessage(webSocketSession,"Sending progress message: " + df.format(d) + "%");
            System.out.println("Sending progress message: " + df.format(d) + "%");
        } else {
        	WebSocketMessageSend.sendMessage(webSocketSession,"Sending progress message: " + transfered);
